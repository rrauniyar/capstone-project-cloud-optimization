...
import com.capstone.beginsetup.model.EC2InstanceInfoThree;
...
@RestController
@RequestMapping("/ec2-instances-three")
public class EC2ControllerCPU3 {
...
@GetMapping("/three-day")
    public ResponseEntity<?> getAllEC2InstanceInfo() {
        try {
            DecimalFormat decimalFormat = new DecimalFormat("#.00");
            List<EC2InstanceInfoThree> instanceInfoList = new ArrayList<>();
            ...
            for (Reservation reservation : reservations) {
                ...
                for (Instance instance : instances) {
                    ...
                    double totalCostForThreeDays = gettotalCostForThreeDays(instanceType);
                    String formattedAverageCpuUtilization = decimalFormat.format(averageCpuUtilization);
                    String formattedTotalCostForThreeDays = decimalFormat.format(totalCostForThreeDays);

                    EC2InstanceInfoThree instanceInfo = new EC2InstanceInfoThree(
                            instanceId,
                            instanceName,
                            instanceType,
                            ...
                            Double.parseDouble(formattedTotalCostForThreeDays));
    ...
    private double getAverageCpuUtilization(String instanceId) {
        GetMetricStatisticsRequest request = new GetMetricStatisticsRequest()
                ...
                .withStartTime(new Date(System.currentTimeMillis() - 3 * 24 * 60 * 60 * 1000)) // Start time: 3 days ago
    ...
    private double gettotalCostForThreeDays(String instanceType) {
        ...
        double totalDays = 3.0; // Total days for cost calculation
